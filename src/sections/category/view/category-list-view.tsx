...
const TABLE_HEAD = [
  { id: 'name', label: 'Name' },
  { id: 'creationDate', label: 'Date Created', width: 180 },
  { id: 'lastModifiedDate', label: 'Date Updated', width: 220 },
  { id: '', width: 88 },
];
...
function applyFilter({ inputData, comparator, filters }: ApplyFilterProps) {
  ...
  if (name) {
    inputData = inputData.filter(
      (user) => user.categoryName.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }

  if (status !== 'all') {
    inputData = inputData.filter((user) => user.categoryName === status);
  }

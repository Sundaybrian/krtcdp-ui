...
const TABLE_HEAD = [
  { id: 'orderNumber', label: 'Date created', width: 88 },
  { id: 'name', label: 'Crop' },
  { id: 'createdAt', label: 'Harvest date', width: 140 },
  {
    id: 'totalQuantity',
    label: 'Quantity',
    width: 120,
...
export function HarvestListView() {
  ...
  const [tableData, setTableData] = useState<Harvest[]>([]);
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading="Harvest List"
          links={[
            { name: 'Dashboard', href: paths.dashboard.root },
            { name: 'Harvest', href: paths.dashboard.order.root },
            { name: 'List' },
          ]}
          sx={{ mb: { xs: 3, md: 5 } }}
        />
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <OrderTableRow
                        ...
                        onViewRow={() => handleViewRow(row.farmerId)}
                      />
                    ))}
...
function applyFilter({ inputData, comparator, filters, dateError }: ApplyFilterProps) {
  ...
  if (name) {
    inputData = inputData.filter(
      (order) =>
        order.crop.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        order.unit.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
  ...
  if (!dateError) {
    if (startDate && endDate) {
      inputData = inputData.filter((order) => fIsBetween(order.harvestDate, startDate, endDate));
    }
  }

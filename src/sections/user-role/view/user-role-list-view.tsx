...
import type { UserRoles } from 'src/api/data.inteface';
...
import { useSearchUserRoles } from 'src/actions/permission';
...
import { RoleTableToolbar } from '../user-role-table-toolbar';
...
export function UserRoleListView() {
  ...
  const { roles, loading } = useSearchUserRoles();
...
function applyFilter({ inputData, filters }: ApplyFilterProps) {
  ...
  if (stock.length) {
    inputData = inputData.filter((product) => stock.includes(product.role.name));
  }

  if (publish.length) {
    inputData = inputData.filter((product) => publish.includes(product.role.name));
  }

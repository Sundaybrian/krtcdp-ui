...
export function ValueChainListView() {
  const confirmRows = useBoolean();

  const router = useRouter();

  const { searchResults, searchLoading } = useSearchValueChain();

  const filters = useSetState<IProductTableFilters>({ publish: [], stock: [] });

  const [tableData, setTableData] = useState<ValueChain[]>([]);
      ...
      <DashboardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Value Chain', href: paths.dashboard.valuechain.root },
            { name: 'List' },
          ]}
          action={
            <Button
...
function applyFilter({ inputData, filters }: ApplyFilterProps) {
  ...
  if (publish.length) {
    inputData = inputData.filter((product) => publish.includes(product.valueChainType));
  }
